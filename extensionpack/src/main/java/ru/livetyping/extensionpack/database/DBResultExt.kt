...
fun <T> databaseSubscribe(response: T?): DBResult<T> =
    if (response == null) databaseEmptyDB else databaseSuccess<T>(response)

fun <T> databaseSubscribeList(response: T?, includeEmptyData: Boolean = true): DBResult<T> {
    return when {
        response == null -> databaseEmptyDB
        ...
        else -> databaseSubscribe(response)
    }
}

inline fun <T> DBResult<T>.handle(
    queryingDB: () -> Unit = {},
    emptyDB: () -> Unit = {},
