...
fun Context.isFragmentWithTagVisible(tag: String): Boolean {
    ...
    val presentFragment = supportFragmentManager.findFragmentByTag(tag)?.isVisible

    return if (presentFragment != null) {
        supportFragmentManager.findFragmentByTag(tag) != null && presentFragment
    } else {
        false
    }
}
...
val Fragment.supportsPictureInPicture: Boolean
    get() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && requireContext().packageManager.hasSystemFeature(
